...
data {
  ...
  real<lower=0,upper=1> PJuv[Nsrv];// Proportion juveniles in surveys
...
parameters {
  ...
  real<lower=-2,upper=3> gamma_A ;// log hazard ratio, adults vs baseline (10% Mort.)
  real<lower=0,upper=3> gamma_Y ; // log hazard ratio, yearlings vs adults
  real<lower=-3,upper=3> gamma_N ;// log hazard ratio, newborns vs baseline (35% Mort.) 
  ...
  vector[Nyrs] epsP ;             // z-score of devs for stoch. effect, Preg
  real<lower=0,upper=3> sig_N ;   // stochasticity in newborn survival (log haz)
  vector[Nyrs] epsN ;             // z-score of devs for stoch. effect, NB
  real gamma_H_mn ;               // mean harvest log hazards  
  real<lower=0,upper=5> sig_H ;   // variance in harvest log hazards 
  vector[NyrsH] epsH ;            // deviations from mean log harvest, z-scores  
  real<lower=0,upper=1> theta_NB ;   // Probability detect newborn carcass
  real<lower=0,upper=1> theta_OA ;   // Probability detect older animal carcass
} 
// Section 3. Additional transformed parameters, including key model dynamics
transformed parameters {
  real gammA = gamma_A - 2.25;         // adult log hazards 
  real gammN = gamma_N - .85;          // newborn log hazards 
  ...
  vector[NyrsH] gamma_H = epsH * sig_H;// deviations from mean log harvest
  vector[NyrsH] gammH ;                // annual harvest log hazards during early years 
  vector[Nyrs] Pr ;                    // pregancy rate 
  vector[Nyrs] S_NN ;                  // Neonatal survival rate
  vector[Nyrs] S_N ;                   // Newborn survival rate (first year)
  ...
  gammH = gamma_H_mn - 5 + gamma_H ;
  // Year 1
  N[1] = exp(8.5+logNinit) ;
  n[1] = N[1] * ssd ;
  Harv_expect[1] = Harv[1] ;
  Pr[1] = inv_logit(alpha + eps_P[1]) ;
  S_NN[1] = exp(-exp(gammN + phi * (N[1]/10000) + eps_N[1])) ;
  S_N[1] = square(S_NN[1]) * sqrt(S_Y) ;
  ...
  for(t in 2:NyrsH){
    ...
    S_NN[t] = exp(-haz_N) ;
    S_N[t] = square(S_NN[t]) * sqrt(S_Y) ;
    M = makemat(NStg,Pr[t],S_NN[t],S_N[t],S_Yh,S_Ah) ;
    n[t] = M * n[t-1]  ;
    N[t] = sum(n[t]) ;
    nt[1:8] = n[t-1][1:8] ;  
    nt[9] = sum(n[t-1][9:12]) ;  
  ...
  for(t in (NyrsH+1):Nyrs){
    ...
    S_NN[t] = exp(-haz_N) ;
    S_N[t] = square(S_NN[t]) * sqrt(S_Y) ;
    M = makemat(NStg,Pr[t],S_NN[t],S_N[t],S_Y,S_A) ;
    n[t] = M * n[t-1]  ;
    N[t] = sum(n[t]) ;
    ...
    nd = D * nt ;
    nd_NB[t] = nd[1] + n[t-1][11] * S_A * (1-S_NN[t]) +.001 ;
...
model {
  ...
  PJuv ~ beta(upsilon*ppn_J[YrSv]+.0001,upsilon*(1-ppn_J[YrSv])+.0001 ) ;
  // -Stranding counts, newborns
  StrNB ~ poisson( theta_NB * nd_NB[YrSt]) ;     
  // -Stranding counts, older animals
  StrOA ~ poisson( theta_OA * nd_OA[YrSt]) ;     
...
generated quantities {
  real S_NN_mn ;                 // mean newborn survival rate 
  real S_N_mn ;                 // mean calf survival rate 
